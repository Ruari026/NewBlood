...
void ANewBloodCharacter::BeginPlay()
{
	...
	if (roleWidgetBP != nullptr)
	{
		if (IsLocallyControlled())
		{
			UUserWidget* roleWidgetInstance = CreateWidget<UUserWidget>(GetWorld(), roleWidgetBP);
			if (roleWidgetInstance != nullptr)
			{
				roleWidgetInstance->AddToViewport();
			}
		}
	}
}
